...
class TensorboardSummary(object):
    ...
    def visualize_test_image(self, writer, dataset, image, target, output, global_step):
        grid_image = make_grid(image[:20].clone().cpu().data, 20, normalize=True)
        writer.add_image('Test Image', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.max(output[:20], 1)[1].detach().cpu().numpy(),
                                                       dataset=dataset), 20, normalize=False, range=(0, 255))
        writer.add_image('Test Predicted label', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.squeeze(target[:20], 1).detach().cpu().numpy(),
                                                       dataset=dataset), 20, normalize=False, range=(0, 255))
